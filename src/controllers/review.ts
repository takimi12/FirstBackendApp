...
export const updateReview = async (req: Request, res: Response) => {
    try {
      ...
      const sanitizedBody: Partial<Omit<Review, "createdAt" | "accepted" | "productId">> = {};
      for (const key of ["name", "email", "message", "rating"] as const) {
        if (body[key] !== undefined) {
          sanitizedBody[key] = body[key] as never; // TS już nie narzeka
        }
      }
...
export const acceptReview = async (req: Request, res: Response) => {
  try {
    const id = req.params?.id;
    if (!id) return res.status(400).json({ message: "ID is required" });
